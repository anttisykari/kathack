...
var VOL_MULT = 1.0,
    ...
    POSSIBLE_TRANSFORM_PREFIXES = ['-webkit-', '-moz-', ''];
...
function StickyNodes() {
    ...
    this.addDomNode = function (n) {
        domNodes.push(n);
    }
    
    this.addBoxes = function (borderstyle) {
        var i, len;
        for (i = 0, len = domNodes.length; i < len; i++) {
            domNodes[i].style.border = borderstyle;
        }
    };
...
function PlayerBall(parentNode, stickyNodes, sounds) {
    var x = 300, y = 300,
        vx = 0, vy = 0,
        radius = 20,
    ...
    this.init = function () {
        ...
        /* test for -webkit-transform or -moz-transform. */
        if (CSS_TRANSFORM === null) {
            getCssTransform();
        }
    };
...
function modifyListeners(obj, add, paramsList) {
    var i, f = (add)? obj.addEventListener : obj.removeEventListener;
    for (i = 0; i < paramsList.length; i++) {
        f.apply(obj, paramsList[i]);
    }
}
...
function Game(gameDiv, stickyNodes) {
    var stickyNodes, player1, physicsInterval, resizeInterval;
    player1 = new PlayerBall(gameDiv, stickyNodes, false);
    player1.init();
...
<input id="bgmusicc" type="checkbox" checked="checked" /></div>\
...
function main() {
    ...
    setTimeout(function () {
        var i, len, bodyNodes = document.body.childNodes;
        stickyNodes = new StickyNodes();
        for (i = 0, len = bodyNodes.length; i < len; i++) {
            if (bodyNodes[i] != gameDiv) {
                stickyNodes.addWords(bodyNodes[i]);
                stickyNodes.addTagNames(bodyNodes[i],
                                        ['img', 'input', 'select']);
            }
        }
        stickyNodes.addBoxes('1px solid #bbb');

        checkInterval = setInterval(function () {
            if (window.jQuery) {
                clearInterval(checkInterval);
                whenAllLoaded(gameDiv, popup, stickyNodes);
            }
        }, 100);
    }, 0);
}
